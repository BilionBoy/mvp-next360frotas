...
class OCategoriasServicoController < ApplicationController
  ...
  def index
    ...
    @pagy, @o_categorias_servico = pagy(@q.result)
  end
  ...
  def create
    ...
    if @o_categoria_servico.save
      redirect_to o_categorias_servico_path, notice: t('messages.created_successfully')
    else
  ...
  def update
    if @o_categoria_servico.update(o_categoria_servico_params)
      redirect_to o_categorias_servico_path, notice: t('messages.updated_successfully'), status: :see_other
    else
  ...
  def destroy
    if @o_categoria_servico.destroy
      redirect_to o_categorias_servico_url, notice: t('messages.deleted_successfully')
    else
      redirect_to o_categorias_servico_url, alert: t('messages.delete_failed_due_to_dependencies')
    end   
  end
  ...
  def set_o_categoria_servico
    ...
    return redirect_to o_categorias_servico_path, alert: t('messages.not_found') unless @o_categoria_servico
  end
  ...
  def handle_not_found
    redirect_to o_categorias_servico_path, alert: t('messages.not_found')
  end
end
