...
class GTiposCentroCustoController < ApplicationController
  ...
  def index
    ...
    @pagy, @g_tipos_centro_custo = pagy(@q.result)
  end
  ...
  def create
    ...
    if @g_tipo_centro_custo.save
      redirect_to g_tipos_centro_custo_path, notice: t('messages.created_successfully')
    else
  ...
  def update
    if @g_tipo_centro_custo.update(g_tipo_centro_custo_params)
      redirect_to g_tipos_centro_custo_path, notice: t('messages.updated_successfully'), status: :see_other
    else
  ...
  def destroy
    if @g_tipo_centro_custo.destroy
      redirect_to g_tipos_centro_custo_url, notice: t('messages.deleted_successfully')
    else
      redirect_to g_tipos_centro_custo_url, alert: t('messages.delete_failed_due_to_dependencies')
    end   
  end
  ...
  def set_g_tipo_centro_custo
    ...
    return redirect_to g_tipos_centro_custo_path, alert: t('messages.not_found') unless @g_tipo_centro_custo
  end
  ...
  def handle_not_found
    redirect_to g_tipos_centro_custo_path, alert: t('messages.not_found')
  end
end
