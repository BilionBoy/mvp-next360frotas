...
class OStatusController < ApplicationController
  ...
  def destroy
    if @o_status.destroy
      redirect_to o_status_index_path, notice: t('messages.deleted_successfully')
    else
      redirect_to o_status_index_path, alert: t('messages.delete_failed_due_to_dependencies')
    end   
  end
  ...
  def set_o_status
    ...
    return redirect_to o_status_path, alert: t('messages.not_found') unless @o_status
  end
  ...
  def handle_not_found
    redirect_to o_status_path, alert: t('messages.not_found')
  end
end
