...
class GStatusController < ApplicationController
  ...
  def create
    ...
    if @g_status.save
     redirect_to g_status_index_path(@g_status), notice: t('messages.created_successfully')
    else
  ...
  def update
    if @g_status.update(g_status_params)
      redirect_to g_status_path, notice: t('messages.updated_successfully'), status: :see_other
    else
...
def destroy
  if @g_status.destroy
    redirect_to g_status_index_path, notice: t('messages.deleted_successfully')
  else
    redirect_to g_status_index_path, alert: t('messages.delete_failed_due_to_dependencies')
  end   
end
